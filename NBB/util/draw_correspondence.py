...
# Modified code
def draw_correspondence(A, B, correspondence, radius, save_dir, level=0, name='', datarootA='', datarootB=''):
    A_marked = util.tensor2im(A)
    B_marked = util.tensor2im(B)
    ...
    A_PIL = Image.fromarray(A_marked)
    A_PIL = A_PIL.resize((A_width, A_height))
    A_PIL.save(os.path.join(save_dir, 'A_level'+str(level)+name+'.png'))
    B_PIL = Image.fromarray(B_marked)
    B_PIL = B_PIL.resize((B_width, B_height))
    B_PIL.save(os.path.join(save_dir, 'B_level'+str(level)+name+'.png'))
